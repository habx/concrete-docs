...
```jsx
  const MyComponent = () => (
    <Button
      onClick={() => notify("I can be called from anwhere !")}
    >
      Click Me!
    </Button>
  )
...
<Button onClick={() => notify("I can be called from anwhere !")}>Click Me!</Button>

### With illustration

```jsx
  const MyComponent = () => (
    <Button onClick={() => notify({ message: 'hi', illustration: <Icon icon='3D-building-outline'/> })}>Click Me!</Button>
  )
```
...
```jsx
  const MyComponent = () => {
    const message = <div><Title type='regular'>Title</Title><Text>Text</Text></div>
    return <Button onClick={() => notify({ message })}>Click Me!</Button>
  }
```

<Button onClick={() => notify({ message: <div><Title type='regular'>Title</Title><Text>Text</Text></div> })}>Click Me!</Button>
...
```jsx
  const MyComponent = () => (
    <Button onClick={() => notify({ message: '**hi in bold**', markdown: true })}>Click Me!</Button>
  )
```
...
<TypeContainer>
  <Button small onClick={() => notify("info", { type: 'info' })}>info</Button>
  <Button small warning onClick={() => notify("warning", { type: 'warning' })}>warning</Button>
  <Button small error onClick={() => notify("error", { type: 'error' })}>error</Button>
</TypeContainer>
