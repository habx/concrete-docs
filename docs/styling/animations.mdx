...
import {ActionBar, animations, Background, Button, Card, palette, theme} from '@habx/ui-core'
...
export const AnimationExample = () => {
  ...
  const animations = ['emerge', 'emergeSlantFromTop', 'emergeSlantFromBottom', 'dive', 'diveSlant']
  const handleSetAnimation = (key) => {
    setAnimation(key)
    setTimeout(() => setAnimation(currentKey => key === currentKey ? null : currentKey), 1000)
  }
  return <div>
    <StyledAnimationExample data-animation={animation} />
    <ActionBar>
      {animations.map(animationKey => (
        <Button outline disabled={animationKey === animation} key={animationKey} small onClick={() => handleSetAnimation(animationKey)}>
          {animationKey}
        </Button>
      ))}
    </ActionBar>
...
<AnimationContainer backgroundColor={palette.neutralWhiteWithIntensityFading[500]}>
  <AnimationExample />
</AnimationContainer>
