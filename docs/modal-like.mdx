...
const LightBoxExample = () => (
  <LightBoxExample triggerElement={<Button>Open the modal</Button>} title='Hi !'>
    I'm a modal
  </LightBoxExample>
)
```
...
*Do not worry about the `onClick` props and ref, we keep it in your triggerElement*
...
const MenuExampleWithComplexTriggerElement = () => (
  <Modal triggerElement={modal => <Button onClick={modal.onClick}>Open the modal ({modal.open ? 'opened' : 'closed'})</Button>} >
    <Text>Hi !</Text>
  </Modal>
)
```
...
If needed, you can also pass a trigger ref instead of a component.
...
const ModalExample = () => {
  ...
  return (
    <div>
      <Modal triggerRef={ref} title='Hi !'>
        <Text>I'm a modal</Text>
      </Modal>
      <Button ref={ref}>Open the modal</Button>
    </div>
  )
}
